...
export default class VideoGameService {
    ...
    getGames(pageSize, pageOffset) {
		return this.http({
			...
			data: "fields name,platforms.name,aggregated_rating,genres.name,summary,involved_companies.company.name,involved_companies.publisher,first_release_date,cover.url,status; limit "+pageSize+"; offset "+pageOffset+";"
		})
		.then(response => {
	...
	getGameById(id) {
		return this.http({
			...
			data: "fields name,platforms.name,aggregated_rating,genres.name,summary,involved_companies.company.name,involved_companies.publisher,first_release_date,cover.url,status; limit 1; where id="+id+";"
		})
		.then(response => {
			const data = response.data.map((d) => { return {
				id: d.id,
